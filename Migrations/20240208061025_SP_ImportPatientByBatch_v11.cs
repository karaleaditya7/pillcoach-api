...
{
    public partial class SP_ImportPatientByBatch_v11 : Migration
    {
...
begin
	...
	declare @id int, @pharmacyId int, @doctorId int, @doctorContactId int, @patientId int, @patientContactId int, @patientAddressId int, @rowError nvarchar(max), @nextFillDate datetime2
		...
		, @batchCount int = 0, @patientNoteId int
	...
	declare
		...
		, @previousFillId int, @scheduledRefillDate date, @inUse bit, @prescribeDate datetime2, @primaryThirdParty nvarchar(100), @primaryThirdPartyBin nvarchar(100)
	...
	begin
		...
		begin try
			select
				...
				, @drugSbdcName = trim(DrugSbdcName), @prescribeDate = RxDate, @primaryThirdParty = PrimaryThirdParty, @primaryThirdPartyBin = PrimaryThirdPartyBin
			from
			...
			begin
				...
				begin
					...
					insert into notes ( [text], lastUpdated )
					values ( '', @now )

					set @patientNoteId = scope_identity()

					insert patient ( pharmacyId, addressId, contactId, [status], importSourceFileId, patientVendorRxID, noteId, language )
					values ( @pharmacyId, @patientAddressId, @patientContactId, 'New Patient', @sourceFileId, @patientIdentifier, @patientNoteId, @patientLanguage )
				...
				select top 1
					@drugSubGroup = trim(value_set_subgroup)
				...
				set @inUse = case when (
					datediff(m, @dateFilled, @now) > @maxMonths
					or (datediff(m, @dateFilled, @now) = @maxMonths and day(@dateFilled) <= day(@now))
				) then 0 else 1 end

				set @nextFillDate = dateadd(d, @daysSupply, @dateFilled)

				set @refillDue = case when @inUse = 1 and @nextFillDate <= @now then 1 else 0 end
				...
				begin
					insert medication (
						...
						, isActive, inUse, refillDue, consumptionEndDate, sbdcName, prescribeDate
						, primaryThirdParty, primaryThirdPartyBin
					) values (
						@patientId, @rxNumber, @refillNumber, @refillsRemaining, @prescribeDate, @dispensedItemName, nullif(@drugGenericName, '')
						...
						, 1, @inUse, @refillDue, @nextFillDate, @drugSbdcName, @prescribeDate, @primaryThirdParty, @primaryThirdPartyBin
					)
					...
					if @isPdcMedication = 1 and dateadd(D, -5, @nextFillDate) >= @now
					begin
		...
		select
			...
			, @isPdcMedication = 0, @drugSbdcName = null, @refillDue = null, @previousFillId = null, @scheduledRefillDate = null, @inUse = null, @patientNoteId = null
			, @prescribeDate = null, @primaryThirdParty = null, @primaryThirdPartyBin = null
	...
	begin
		...
		update ImportSourceFiles set
			ImportStatusId = case
				...
				when TotalImported = 0 then 9 -- Import Failed
	...
	set nocount off
        ...
        {
            migrationBuilder.Sql("IF OBJECT_ID('[dbo].[ImportPatientByBatch]', 'P') IS NOT NULL DROP PROCEDURE [dbo].[ImportPatientByBatch];");
        }
    }
}

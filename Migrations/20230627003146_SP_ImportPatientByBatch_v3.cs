...
{
    public partial class SP_ImportPatientByBatch_v3 : Migration
    {
...
begin
	...
	declare
		...
		, @dispensedItemName nvarchar(max), @directions nvarchar(max), @patientPaidAmount decimal(18, 2), @isPdcMedication bit
			...
			begin
				...
				begin
					insert medication (
						...
						, isActive, inUse, refillDue
					) values (
						@patientId, @rxNumber, @refillNumber, @refillsRemaining, @dateFilled, trim(@dispensedItemName), nullif(trim(@drugGenericName), '')
					...
					begin
						...
						begin
							insert medicationConsumptions (
								patientId, rxNumber, [date], [status], drugSubGroup, ndcNumber, condition, importSourceFileId, medicationId
							) values (
								@patientId, @rxNumber, @date, 1, @drugSubGroup, @dispensedItemNDC, @medicationCategory, @sourceFileId, @medicationId
							)

...
{
    public partial class SP_ImportPatient : Migration
    {
...
begin
	...
	begin
		...
		begin try
			...
			begin
				...
				from
					Pdc_Medications where code = @dispensedItemNDC and trim(code_type) = 'NDC'
				order by Id desc

				if isnull(@drugSubGroup, '') <> '' and isnull(@medicationCategory, '') <> ''
				begin
					set @nextFillDate = dateadd(d, @daysSupply, @dateFilled)
					set @excludeMedication = case when @medicationCategory in ('Cholesterol', 'Diabetes', 'RASA') then 0 else 1 end
					...
					if @medicationId is null or @lastRefillNumber <> @refillNumber
					begin
						insert medication (
							...
							, @dateFilled, @nextFillDate, convert(int, @patientPaidAmount), @dispensedItemNDC, @drugSubGroup, @medicationCategory, @sourceFileId
							, @doctorId, @excludeMedication, convert(bit, 1 - @excludeMedication)
						)

						-- insert/update medication consumptions

						if @excludeMedication = 0
						begin
		...
		select
			...
			, @drugSubGroup = null, @medicationCategory = null, @excludeMedication = null, @surplusDays = null, @drugGenericName = null
	...
	begin
		select @errorsJson = (select * from @errors for json auto, root('importErrors'))
		set @importStatusId = 8 -- Partially Imported
	end

	if @successCount = 0 set @importStatusId = 7 -- Failed

	update ImportSourceFiles set ImportStatusId = @importStatusId, ImportEndTimeUTC = getutcdate(), ErrorsJson = @errorsJson where Id = @sourceFileId
	...
	set nocount off
        ...
        {
            var sqlScript = @"drop procedure if exists [ImportPatient]";

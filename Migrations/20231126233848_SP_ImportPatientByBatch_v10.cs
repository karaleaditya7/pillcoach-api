...
{
    public partial class SP_ImportPatientByBatch_v10 : Migration
    {
...
begin
	...
	declare
		...
		, @previousFillId int, @scheduledRefillDate date, @inUse bit, @prescribeDate datetime2
	...
	begin
		...
		begin try
			select
				...
				, @drugSbdcName = trim(DrugSbdcName), @prescribeDate = RxDate
			from
			...
			begin
				...
				begin
					...
					insert patient ( pharmacyId, addressId, contactId, [status], importSourceFileId, patientVendorRxID, noteId )
					values ( @pharmacyId, @patientAddressId, @patientContactId, 'New Patient', @sourceFileId, @patientIdentifier, @patientNoteId )
				...
				begin
					insert medication (
						...
						, 1, @inUse, @refillDue, @nextFillDate, @drugSbdcName, @prescribeDate
					)
...
begin
	...
	declare
		...
		, @rxNumber nvarchar(50), @refillNumber int, @dateFilled datetime2, @daysSupply int, @refillsRemaining int, @dispensedQuantity decimal(18, 2), @dispensedItemNDC nvarchar(50)
		, @dispensedItemName nvarchar(max), @directions nvarchar(max), @patientPaidAmount decimal(18, 2), @isPdcMedication bit, @drugSbdcName nvarchar(max), @refillDue bit
		, @previousFillId int, @scheduledRefillDate date, @inUse bit
			...
			begin
				...
				begin
					insert medication (
						...
						, @dateFilled, @nextFillDate, @patientPaidAmount, @dispensedItemNDC, @drugSubGroup, @medicationCategory
						, @sourceFileId, @doctorId, @excludeMedication, convert(bit, 1 - @excludeMedication)
						, 1, @inUse, @refillDue, @nextFillDate, @drugSbdcName
					)

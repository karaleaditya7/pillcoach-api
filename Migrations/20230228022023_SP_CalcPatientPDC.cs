...
{
    public partial class SP_CalcPatientPDC : Migration
    {
...
begin
	...
	begin
		...
		if @months = 6 set @startDate = dateadd(d, 1, dateadd(m, -@months, @endDate))
		...
		if not exists (
			...
			select * from Pdc_Medications where category in ('Insulins', 'Sacubitril_Valsartan') and code in (
				select distinct ndcNumber from medication where patientId = @patientId and lastFillDate between @startDate and @endDate and condition = @condition
			)
		)
		begin
	...
	set nocount off
        ...
        {
            var sqlScript = @"drop procedure if exists [CalcPatientPDC]";
